...
/**
 * check_elf - This checks a file if its an ELF file.
 ...
 * @e_ident: Is a pointer to an array that contains the ELF magic class
 */
void print_data(unsigned char *e_ident)
{
	printf("Data:");
 ...
 * print_version - This prints the version of an ELF header.
 @e_ident: Is a pointer to an array that contains the ELF magic version
 */
void print_version(unsigned char *e_ident)
{
	printf("Version: %d",
 ...
 * @e_ident: Pointer array that contains the ELF ABI version.
 */
void print_abi(unsigned char *e_ident)
{
...
/**
 * close_elf - This is the close of ELF file.
...
int main(int __attribute__((__unused__)) argc, char *argv[])
{
	...
	r = read(w, header, sizeof(Elf64_Ehdr));
	if (r == -1)
	{
		free(header);
		close_elf(w);
